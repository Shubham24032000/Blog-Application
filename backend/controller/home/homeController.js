...
module.exports.home_article_get = async(req,res)=>{
    ...
    try {
        ...
        if(searchValue){
            ...
            articles = await articleModel.find({}).skip(skipPage).limit(parPage).sort({createdAt:-1});
        }
...
module.exports.old_react_article = async(req,res) =>{
    try {
        ...
        const recentArticle = await articleModel.find({}).limit(3).sort({createdAt:-1})
        return res.status(200).json({
            oldArticle,
            recentArticle
        })
...
module.exports.category_article_get = async(req,res) =>{
    ...
    try {
        const countArticle = await articleModel.find({category_slug:categorySlug}).countDocuments();
          const articles = await articleModel.find({category_slug:categorySlug}).skip(skipPage).limit(parPage).sort({createdAt:-1});
        return res.status(200).json({
...
module.exports.tag_article_get = async(req,res) =>{
    ...
    try {
        const countArticle = await articleModel.find({tag_slug:tagSlug}).countDocuments();
          const articles = await articleModel.find({tag_slug:tagSlug}).skip(skipPage).limit(parPage).sort({createdAt:-1});
        return res.status(200).json({
            tagArticle: articles,
            parPage,
