...
export const edit_category = (categorySlug) => async(dispatch) => {
    try {
        const response = await axios.get(`http://localhost:4000/rest-api/edit-category/${categorySlug}`,{withCredentials:true});
        dispatch({
...
export const update_category = (id,data) => async(dispatch) => {
    try {
        const response = await axios.patch(`http://localhost:4000/rest-api/update-category/${id}`,data,{withCredentials:true});
        dispatch({
