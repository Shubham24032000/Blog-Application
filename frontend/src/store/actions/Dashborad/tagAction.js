...
export const edit_tag = (categorySlug) => async(dispatch) => {
    try {
        const response = await axios.get(`http://localhost:4000/rest-api/edit-tag/${categorySlug}`,{withCredentials:true});
        dispatch({
...
export const update_tag = (id,data) => async(dispatch) => {
    try {
        const response = await axios.patch(`http://localhost:4000/rest-api/update-tag/${id}`,data,{withCredentials:true});
        dispatch({
