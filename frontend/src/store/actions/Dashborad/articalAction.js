...
export const get_all_article = (currentPage,searchValue)=>async(dispatch)=>{
    try {
        const response = await axios.get(`http://localhost:4000/rest-api/get-artical?currentPage=${currentPage}&&searchValue=${searchValue}`,{withCredentials:true});
        dispatch({
...
export const article_edit = (articleSlug)=>async(dispatch)=>{
    ...
    try {
        const response = await axios.get(`http://localhost:4000/rest-api/edit-artical/${articleSlug}`,{withCredentials:true});
        dispatch({
...
export const article_update = (data)=>async(dispatch)=>{
    ...
    try {
        ...
        const response = await axios.post('http://localhost:4000/rest-api/update-artical',data,{withCredentials:true});
...
export const delete_article = (articleId)=>async(dispatch)=>{
    try {
        const response = await axios.delete(`http://localhost:4000/rest-api/delete-artical/${articleId}`,{withCredentials:true});
        dispatch({
