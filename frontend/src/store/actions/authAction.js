...
export const register = (data) =>async(dispatch)=>{
   ...
   try {
      const response = await axios.post('http://localhost:4000/rest-api/user-register',data,{withCredentials:true});
      dispatch({
...
export const verify_email = (otp) =>async(dispatch)=>{
   dispatch({type:'LOADER_RUN'})
   try {
      const response = await axios.post('http://localhost:4000/rest-api/verify-email',{otp},{withCredentials:true});
      localStorage.setItem('blog_token',response.data.token);
      dispatch({
...
export const user_login = (data) =>async(dispatch)=>{
   ...
   dispatch({type:'LOADER_RUN'})
   try {
      const response = await axios.post('http://localhost:4000/rest-api/user-login',data,{withCredentials:true});
...
export const logout_user = ({role,history}) =>async(dispatch)=>{
   ...
   try {
      await axios.get('http://localhost:4000/rest-api/logout-user',{withCredentials:true});
