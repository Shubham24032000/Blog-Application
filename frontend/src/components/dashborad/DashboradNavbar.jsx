...
const DashboradNavbar = () => {
   ...
   <>
    ...
    <Toaster position={'bottom-center'}
        ...
        <h2><Link to='/dashborad'>Shubham Tiwari</Link></h2>
        </div>
            ...
            <div className="natification-message">
                <div className="natification">
                    <div onClick={nModel}>
                        <span><BsBell/></span>
                        {
                            notifications.length > 0 && <div className="nCount">
                    ...
                    <div className={`natifications ${nModelShow ? 'show' : ''}`}>
                        <ul>
                            {
                                notifications.map((n,i)=><li className={n.status === 'seen'?'':'bg'} key={i}><Link onClick={()=>{seenNotification(n._id)}} to={`/artical/details/${n.slug}`}>{n.subject}</Link>
