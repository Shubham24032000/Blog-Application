    ...
    return(
        <div className="article-details">
            ...
            <div className="home-artical-text">
                <p>{htmlToText(read_article?.articleText)}
                </p>
            </div>
            <div className="like-dislike-view">
                <div className="like-dislike">
                    <div className="dislike">
                        {
                            userInfo && userInfo.role === 'user' ? <button onClick={article_dislike}className={dislike_status === 'dislike' ? 'icon red' : 'icon'}><AiFillDislike/></button> :
                    ...
                    <div className="like">
                        {
                            userInfo && userInfo.role === 'user' ? <button onClick={article_like} className={like_status === 'like' ? 'icon blue' : 'icon'}><AiFillLike/></button> :
            ...
            <div className="more-tags">
                <span>Tags</span>
                {
                    moreTag.length > 0 && moreTag.map((teg,index)=>
                    <Link to={`/artical/tag/${teg}`} key={index}>{teg.split('-').join('')}</Link>
                    )
                }
            </div>
            ...
            <div className="related-article">
                ...
                <div className="articles">
                    {
                        related_article.length > 0 ? related_article.map((art,index)=>
                        <Link key={index} to={`/artical/details/${art.slug}`} className='article'>
                            <img src={`http://localhost:3000/articalImage/${art?.image}`} alt=""/>
                    <span>Lorem ipsum dolor sit amet consectetur adipisicing elit. Earum, inventore!</span></Link>
